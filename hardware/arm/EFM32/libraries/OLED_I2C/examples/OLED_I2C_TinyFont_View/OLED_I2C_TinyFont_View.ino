...
{
  myOLED.begin();
  myOLED.setFont(TinyFont);  /* TinyFont / SmallFont /MediumNumbers / BigNumbers */
}

void loop()
{
  myOLED.printxy(" !\"#$%&'()*+,-./", CENTER, 16);
  myOLED.printxy("0123456789:;<=>?", CENTER, 22);
  myOLED.printxy("@ABCDEFGHIJKLMNO", CENTER, 28);
  myOLED.printxy("PQRSTUVWXYZ[\\]^_", CENTER, 34);
  myOLED.printxy("`abcdefghijklmno", CENTER, 40);
  myOLED.printxy("pqrstuvwxyz{|}~ ", CENTER, 46);
  myOLED.update();
